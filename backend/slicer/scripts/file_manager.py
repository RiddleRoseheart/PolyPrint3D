...
class FileManager:
    def __init__(self, host, username, password, remote_path, local_output_path="backend/slicer/output"):
        ...
        """ self.local_output_path = Path(local_output_path)  # Path to save output files, remove when being able to connect with the server """
    ...
    def generate_unique_folder_name(self, prefix="job"):
        ...
        """
        # For local testing, check local directory instead
        try:
            existing_folders = [
    ...
    def get_job_file_path(self, job_name, file_type, group_number):
        ...
        # return f"/dummy/path/{job_name}/{file_type}/{file_name}"  # Dummy remote path for local testing
